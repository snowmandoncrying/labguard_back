...
def get_chat_logs(session_id: str, db: Session = Depends(get_db)):
    return chat_log_crud.load_chat_logs(db, session_id)

# 최근 10개 이어쓰기용
@router.get("/continue/{session_id}", response_model=List[ChatLogOut])
def continue_chat_logs(session_id: str, db: Session = Depends(get_db)):
    return chat_log_crud.continue_chat_logs(db, session_id)
