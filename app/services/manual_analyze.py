...
def extract_risk_chunks(manual_id: str, chunk_text_sample: str = "") -> str:
    ...
    prompt = f"""
당신은 실험 매뉴얼을 분석하여 **위험 요소를 식별하고, 숨겨진 위험성까지 추론하는 전문가**입니다.

아래 실험 매뉴얼 텍스트를 검토하여 다음 두 가지를 모두 수행해주세요:

---

① **명시된 위험 문장 추출**  
- 기존 매뉴얼 안에 이미 나타난 **위험/주의/안전 관련 문장**을 그대로 추출  
- 예: "가열 시 절대 용기를 밀폐하지 마세요."

② **추론을 통한 잠재적 위험 문장 추가**  
- 매뉴얼에 명시되진 않았지만, 등장하는 **물질, 장비, 절차**가 위험할 경우  
- LLM의 지식을 바탕으로 **적절한 경고 문장을 새롭게 생성**  
- 예: "삼산화비소는 발암성 물질로 분류되므로 밀폐된 공간에서 사용을 피하고, 보호장비를 착용해야 합니다."

---

**위험 관련 문장의 기준**:
- 독성, 폭발성, 부식성 등 물질의 위험성
- 고온, 고압, 고속 회전체 등 기구의 위험성
- 사고 예방, 보호장비 착용, 경고/주의/안전 관련 설명
- 키워드 예시: "위험", "주의", "경고", "조심", "반드시", "절대", "안전"

---

**제외할 내용**:
- 단순한 실험 순서 ("용액을 넣는다" 같은 일반 절차)
- 구체적 위험이 없는 반복 설명
